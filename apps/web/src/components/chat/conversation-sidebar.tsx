...
import { Plus, Search, PanelLeftClose, PanelLeft, X } from "lucide-react";
...
export function ConversationSidebar() {
  ...
  const closeOrCollapse = isDesktop
    ? toggleSidebar
    : () => setMobileNavOpen(false);
    ...
    <div className="flex flex-col h-full w-72">
      ...
      <div className="flex items-center justify-between px-3 pt-3 pb-2">
        ...
        <button
          ...
          aria-label={isDesktop ? "Collapse sidebar" : "Close navigation"}
        >
          {isDesktop ? (
  ...
  // Mobil (<lg): overlay drawer — mobileNavOpen ile kayar.
  if (!isDesktop) {
    return (
      <motion.aside
        animate={{ x: mobileNavOpen ? 0 : -300 }}
        transition={{ duration: 0.2, ease: "easeInOut" }}
        className="fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-border bg-background overflow-hidden"
      >
        {panelContent}
      </motion.aside>
    );
  ...
  // Masaüstü (≥lg): kalıcı sidebar — daraltılabilir.
  return (
    <>
      ...
      {sidebarCollapsed && (
        <div className="flex flex-col items-center border-r border-border bg-background px-1.5 pt-3 shrink-0">
          <button
